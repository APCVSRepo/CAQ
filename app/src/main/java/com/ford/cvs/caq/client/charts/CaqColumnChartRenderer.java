 ...
 */
public class CaqColumnChartRenderer extends AbstractChartRenderer {
    ...
    public CaqColumnChartRenderer(Context context, Chart chart, ColumnChartDataProvider dataProvider) {
        super(context, chart);
...
//                highlightColumnsForSubcolumns(canvas);
    ...
    private void calculateMaxViewport() {
        ...
        tempMaximumViewport.set(-0.5f, MAX_Y, data.getColumns().size() - 0.5f, baseValue);
        if (data.isStacked()) {
    ...
    private void drawSubcolumn(Canvas canvas, Column column, SubcolumnValue columnValue, boolean isStacked) {
        canvas.drawRoundRect(drawRect, drawRect.width()/2, drawRect.width()/2, columnPaint);
        if (column.hasLabels()) {
    ...
    private void highlightSubcolumn(Canvas canvas, Column column, SubcolumnValue columnValue, int valueIndex,
        ...
        if (selectedValue.getSecondIndex() == valueIndex) {
            ...
            RectF tmpRect = new RectF(drawRect.left - touchAdditionalWidth, drawRect.top,
                    drawRect.right + touchAdditionalWidth, drawRect.bottom);
            canvas.drawRoundRect(tmpRect, tmpRect.width()/2, tmpRect.width()/2, columnPaint);
